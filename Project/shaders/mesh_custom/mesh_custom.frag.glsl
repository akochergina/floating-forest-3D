...
void main()
{
	...
	//просто сделать все одним цветом
	// FragColor = vec4(0.16, 0.75, 0.63, 1.0);

	//типа источника освещения + как-то модифицируется цвет...
	 //FragColor = abs(vec4(cos(fragment.position.x), 0.16, 0.75, 0.63));

	//короче каждая грань окрашивается в свой цвет (как определяются цвета x, y, z?)
	//FragColor = abs(vec4(N.x, N.y, N.z, 0.0));
	// rem. N represente la normale de la surface (dont la norme est 1).

	/*
	//полосочки другого цвета?? цвет тоже меняется
