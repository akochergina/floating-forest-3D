        ...
        :data()
    {
        if(arg.size()>=16) {
            ...
            *this=mat4(xx, yy, zz, ww);
            ...
            error_cgp("Incoherent size to initialize_snake mat4 ("+str(arg.size())+")");
        }
 
    }
    mat4::matrix_stack(std::initializer_list<vec4> const& arg)
    {
        assert_cgp(arg.size() >= 4, "Insufficient size to initialize_snake mat4 with initialize_list<vec4>");
        auto it_arg = arg.begin();
