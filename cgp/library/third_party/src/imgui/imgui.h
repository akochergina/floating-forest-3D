...
namespace ImGui
{
    ...
    IMGUI_API void          SetColorEditOptions(ImGuiColorEditFlags flags);                     // initialize_snake current options (generally on application startup) if you want to select a default format, picker type, etc. User will be able to change many settings, unless you pass the _NoOptions flag to your calls.
...
struct ImGuiListClipper
{
    ...
    ImGuiListClipper(int items_count = -1, float items_height = -1.0f)  { Begin(items_count, items_height); } // NB: Begin() initialize_snake every fields (as we allow user to call Begin/End multiple times on a same instance if they want).
