...
namespace cgp
{
        ...
        :data()
    {
        if(arg.size()>=9) {
            ...
            *this=mat3(xx, yy, zz);
            ...
            error_cgp("Incoherent size to initialize_snake mat3 with initialize_list<float> ("+str(arg.size())+")");
        }
    ...
    mat3::matrix_stack(std::initializer_list<vec3> const& arg)
        :data()
    {
        assert_cgp(arg.size() >= 3, "Insufficient size to initialize_snake mat3 with initialize_list<vec3>");
        auto it_arg = arg.begin();
