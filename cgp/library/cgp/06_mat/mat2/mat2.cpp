...
namespace cgp
{
        ...
        :data()
    {
        if(arg.size()>=4) {
            ...
            *this=mat2(xx, yy);
            ...
            error_cgp("Incoherent size to initialize_snake mat2 with initialize_list<float> ("+str(arg.size())+")");
        }
    ...
    mat2::matrix_stack(std::initializer_list<vec2> const& arg)
        :data()
    {
        assert_cgp(arg.size() >= 2, "Insufficient size to initialize_snake mat2 with initialize_list<vec2>");
        auto it_arg = arg.begin();
